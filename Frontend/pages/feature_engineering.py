"""
FuelIQ — Feature Engineering Page (v4)
"""
...
def render():
    ...
    <div class="page-header">
        ...
        <div>
            ...
            <div class="page-sub">Engineered features that power the Random Forest model</div>
        </div>
    </div>
    ...
    <div class="fiq-card-accent">
        <div style="font-size:0.86rem;color:{t['text_secondary']};line-height:1.8;">
            ...
            petrol pump logs. These capture stock dynamics, temporal patterns, and lagged sales
            signals.<br>
    ...
    # ── Feature Importance Chart (from actual selected_features.csv) ──────────
    feat_df = load_selected_features()
    has_corr = "Correlation_with_Target" in feat_df.columns and feat_df["Correlation_with_Target"].abs().sum() > 0

    col_ch, col_table = st.columns([3, 2], gap="medium")

    with col_ch:
        st.markdown('<div class="fiq-card" style="padding:16px;">', unsafe_allow_html=True)
        st.markdown('<div class="fiq-card-header">MODEL FEATURE IMPORTANCE</div>',
                    unsafe_allow_html=True)
    ...
    with col_table:
        st.markdown('<div class="fiq-card" style="padding:16px;">', unsafe_allow_html=True)
        st.markdown('<div class="fiq-card-header">IMPORTANCE SCORES</div>', unsafe_allow_html=True)
        ...
        for _, row in top.iterrows():
            ...
            corr_color = t["danger"] if corr < -0.1 else (t["success"] if corr > 0.1 else t["text_secondary"])
            corr_str   = f'<span style="color:{corr_color};">{corr:+.3f}</span>' if has_corr else ""
            st.markdown(f"""
    ...
    GROUPS = {
        "📦 Stock Features": [
            ("Stock_Ratio", "Continuous", "Closing_Stock / Opening_Stock",
             "Core signal — near-zero ratio strongly predicts next-day refill."),
            ("Prev_Closing", "Lagged", "df['Closing_Stock'].shift(1)",
             "Yesterday's closing stock provides continuity context."),
            ("Days_Since_Refill", "Counter", "Resets to 0 on each Refill_Required == Yes day",
             "#1 importance feature. Long streaks signal impending refill."),
        ],
        "📈 Sales Rolling Features": [
            ("Rolling_7d_Sales", "Rolling", "df['Total_Sold'].rolling(7).mean()",
             "Smooths daily noise — captures weekly demand momentum."),
            ("Prev_Total_Sold", "Lagged", "df['Total_Sold'].shift(1)",
            ...
            for feat_name, badge, formula, why in features:
                ...
                <div class="feat-row" style="border-color:{bc}30">
                    ...
                    <div>
                        <div class="feat-name">
                            {feat_name}
                            {"&nbsp;<span style='font-size:0.65rem;color:" + t['success'] + ";'>✓</span>"
                             if in_df else ""}
                        </div>
    ...
    # ── Dataset column presence check ─────────────────────────────────────────
    st.markdown(f'<div style="font-size:0.88rem;font-weight:700;'
                f'color:{t["text_primary"]};margin-bottom:10px;">📋 Loaded Dataset Columns</div>',
                unsafe_allow_html=True)
    expected = ["Date", "Opening_Stock", "Closing_Stock", "Total_Sold", "Target",
                "Rolling_7d_Sales", "Days_Since_Refill", "Stock_Ratio", "DayOfWeek",
                "Is_Weekend", "Month", "Day"]
    cols_status = []
    for col in expected:
        present = col in df.columns
        cols_status.append({"Column": col,
                            "Present": "✅ Yes" if present else "❌ Missing",
                            "Sample": str(df[col].iloc[-1]) if present else "—"})

...
1. **Always run notebooks in order** (01 → 05). Each depends on the previous notebook's output files in `../data/`.
2. **Clear outputs before committing** — large cell outputs (especially from `03` and `04`) bloat notebook file sizes significantly.
3. **Do not commit `.ipynb_checkpoints/`** — these are Jupyter's auto-save duplicates; they are excluded via `.gitignore`.
4. **Document your experiments** — add markdown cells explaining your rationale when modifying any notebook, especially hyperparameter changes in `04`.
5. **Keep training data in `../data/`** — never hardcode absolute paths inside notebooks; always use relative paths (`../data/filename`).
6. **Validate feature order** — if you add or remove features, update `model_features.json` immediately, or inference in the dashboard will silently break.
...
1. Add the engineering logic in `02_feature_selection.ipynb` (new cell in the feature engineering section).
2. Update `model_features.json` to include the new feature name in the correct position.
3. Re-run notebooks `02` → `04` to retrain and re-evaluate.
4. Update `_normalise()` in `Frontend/utils/data_loader.py` to compute the feature at runtime.
5. Update the Feature Engineering dashboard page to document the new feature.

### Retraining with New Data

1. Place the new Excel file in `../data/`.
2. Update the file path reference in `01_data_preparation.ipynb`.
3. Re-run notebooks `01` → `05` in order.
4. The dashboard will automatically use the new `final_model.pkl` and `model_metrics.json`.

### Changing the Model Algorithm

1. Open `04_model_training.ipynb`.
2. Replace the `RandomForestClassifier` import and instantiation.
3. Update `model_metrics.json` with the new scores.
4. Update the "About the Model" section in `Frontend/pages/model_insights.py`.
